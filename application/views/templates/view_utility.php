<?php
    ...
    $format = "%Y-%m-%d";
    $tariff['due_date'] = mdate($format);
    $date_expire = $tariff['due_date'];
    $date = new DateTime($date_expire);
    $now = new DateTime();
    $tdays_left = $date->diff($now)->format("%d");
    $format1 = "%Y-%m-%d";
    $directive['due_date'] = mdate($format1);
    $date_expire = $directive['due_date'];
    $date = new DateTime($date_expire);
    $now = new DateTime();
    $ddays_left = $date->diff($now)->format("%d");

    $format2 = "%Y-%m-%d";
    $project['due_date'] = mdate($format2);
    $date_expire = $project['due_date'];
    $date = new DateTime($date_expire);
    $now = new DateTime();
...
<?php
    $query = $this->db->get_where('cus', array('cu_id' => $cuid));
    $row = $query->row();
?>
        ...
        <div class="row utility">
            <div class="col-lg-12 no-gutter">
                ...
                <?php if (count($indicators) > 0)
                    if (isset($indicators) && $indicators) : $i = 0;
                        foreach ($indicators as $indicator) : $in_id = $indicator->in_id; ?>
                            <div class="col-lg-2">
            ...
            <div class="col-lg-12">
                <div class="tabs-container">
                    <ul class="nav nav-tabs">
                        <?php if (isset($indicators) && $indicators) : $i = 0;
                            foreach ($indicators as $indicator) : ?>
                                <li class="tab<?php if ($i === 0): ?> active<?php endif; ?>">
                                    <a data-toggle="tab"
                                       href="#tab-<?= $indicator->in_id; ?>"><?= $indicator->sname; ?></a>
                                </li>
                                <?php $i++; endforeach; endif; ?>
                                                            ...
                                                            <td style="width: 80px;">
                                                                <?php if ($this->ion_auth->is_admin()) { ?>
                                                                    ...
                                                                    <div class="btn-group">
                                                                        ...
                                                                        <a href="<?php echo base_url() . "indicator/edit_directive/" . $directive->dir_id; ?>"
                                                ...
                                                if (count($tariffs) > 0)
                                                    foreach ($tariffs as $tariff) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><?= $tariff->weight; ?></td>
                                                            <td class="project-status"><span class="label label-danger">Over Due</span>
                                                            </td>
                                                            <td style="width: 80px;">
                                                                <div class="btn-group">
                                                                    <button type="button" data-toggle="tooltip"
